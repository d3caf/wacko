defmodule WackoWeb.TablesLive.Game do
  ...
  def handle_event("draw_from_deck", _value, socket) do
    game = GameServer.draw_from_deck(game_name(socket), socket.assigns.player)
    {:noreply, assign(socket, :game, game)}
  end

  def handle_event("draw_revealed", _value, socket) do
    game = GameServer.draw_revealed_card(game_name(socket), socket.assigns.player)
    {:noreply, assign(socket, :game, game)}
  end
  ...
  defp get_player(%{assigns: %{game: %{players: players}, player: %{name: name}}}) do
